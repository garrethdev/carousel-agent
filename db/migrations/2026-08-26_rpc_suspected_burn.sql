-- Burned-accounts fix 4/5: teach run_account_health_check() the new verdicts.
-- - map suspected_burn to its own account_events row; collapsing already maps to collapse_detected
-- - count collapsing/suspected_burn as recoverable states
-- - carry suppressed_share_7d/14d + deliv_fail_7d into health_detail, health_evidence, and the
--   accounts JSON so the detector email (fix 5) can render the tail
-- - sort actionable burn states to the top of the accounts list
-- Advisory only; does not touch is_active / posting_paused. muted hysteresis unchanged.
...
begin
  ...
  select v.id, v.username, v.geelark_profile, v.platform, v."character",
         ...
         v.suppressed_share_7d, v.suppressed_share_14d, v.deliv_fail_7d,
  ...
  select geelark_profile, username,
         case when new_status = 'banned' then 'banned'
              ...
              when new_status = 'suspected_burn' then 'suspected_burn'
              when new_status = 'healthy' and old_status in ('muted','collapsing','suspected_burn','watch','banned') then 'recovered'
              else 'health_change' end,
         platform, warmup_source,
         format('%s -> %s (med7d=%s med28d=%s sup7=%s sup14=%s cohort=%s n=%s conf=%s)',
                coalesce(old_status,'(new)'), new_status, median_7d_r, median_28d_r,
                suppressed_share_7d, suppressed_share_14d,
  ...
  update accounts a set
     ...
     health_detail = format('med7d=%s med28d=%s sup7=%s sup14=%s cohort=%s matured7d=%s age=%sd band=%s',
                            n.median_7d_r, n.median_28d_r, n.suppressed_share_7d, n.suppressed_share_14d,
                            round(coalesce(n.cohort_median_7d,0)),
     ...
     health_evidence = jsonb_build_object(
        ...
        'suppressed_share_7d', n.suppressed_share_7d, 'suppressed_share_14d', n.suppressed_share_14d,
        'deliv_fail_7d', n.deliv_fail_7d,
  ...
  select jsonb_build_object(
    ...
    'accounts', (select jsonb_agg(jsonb_build_object(
                   ...
                   'suppressed_share_7d',suppressed_share_7d,'suppressed_share_14d',suppressed_share_14d,
                   'deliv_fail_7d',deliv_fail_7d,
                 ...
                 order by array_position(array['banned','suspected_burn','collapsing','tracking broken','no data','muted','watch','idle','ramping','healthy'], new_status), username)
                 from _new),
