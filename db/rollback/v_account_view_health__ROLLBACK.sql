-- ROLLBACK: original definition of v_account_view_health as of 2026-08-26
-- (before the burned-accounts tail-metric change). Re-run this to revert.
CREATE OR REPLACE VIEW public.v_account_view_health AS
 WITH perf AS (
         SELECT post_performance.account, post_performance.posted_at, post_performance.views, 'instagram'::text AS pf
           FROM post_performance
        UNION ALL
         SELECT tt_post_performance.account, tt_post_performance.posted_at, tt_post_performance.views, 'tiktok'::text AS pf
        ...
        UNION ALL
         ...
         SELECT a.id, a.username, a.geelark_profile, a.platform, a."character", a.is_active, a.account_created_on,
            COALESCE(f.fired_7d, 0::bigint) AS fired_7d, COALESCE(f.fired_28d, 0::bigint) AS fired_28d, f.last_fired_at,
          ...
          GROUP BY a.id, a.username, a.geelark_profile, a.platform, a."character", a.is_active, a.account_created_on, f.fired_7d, f.fired_28d, f.last_fired_at
        ), banded AS (
         SELECT g.id, g.username, g.geelark_profile, g.platform, g."character", g.is_active, g.account_created_on,
            g.fired_7d, g.fired_28d, g.last_fired_at, g.posts_7d, g.posts_28d, g.mat_posts_7d, g.mat_posts_28d,
            g.median_7d, g.median_28d, g.max_14d, g.zero_21d, g.last_post_at, g.perf_rows_7d,
 ...
 SELECT b.id, b.username, b.geelark_profile, b.platform, b."character", b.is_active, b.posts_7d, b.posts_28d,
    ...
    round(b.median_7d) AS median_7d_r, round(b.median_28d) AS median_28d_r,
        CASE
            ...
            WHEN b.fired_7d >= 2 AND b.perf_rows_7d = 0 AND b.platform_last_ingest IS NOT NULL AND b.platform_last_ingest >= (now() - '4 days'::interval) THEN 'tracking broken'::text
    ...
    b.fired_7d, b.fired_28d, b.last_fired_at, b.perf_rows_7d, c.cohort_median_7d, c.cohort_n, b.platform_last_ingest,
