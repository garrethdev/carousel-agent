...
const rawPool = fmt.hashtag_pool || fmt.hashtag_stack || '';
let hashtagPool = (Array.isArray(rawPool) ? rawPool : String(rawPool).split(/\s+/))
  .map(t => String(t).trim())
  .filter(Boolean)
  .map(t => (t.startsWith('#') ? t : '#' + t));
if (!hashtagPool.length && fmt.hashtags !== false) hashtagPool = DEFAULT_HASHTAG_POOL.slice();
const hashtagCount = fmt.hashtag_count != null ? fmt.hashtag_count : DEFAULT_HASHTAG_COUNT;
const hashtagAlways = (fmt.hashtag_always || []).map(t => (String(t).startsWith('#') ? String(t) : '#' + t));
...
const processRow = async (row) => {
  ...
  try {
    ...
    if (hashtagPool.length || hashtagAlways.length) {
      const chosen = hashtagAlways.concat(sample(hashtagPool.filter(t => hashtagAlways.indexOf(t) === -1), hashtagCount));
      if (chosen.length) text = text + '\n\n' + sample(chosen, chosen.length).join(' ');
    }
...
return [{
  json: {
    ...
    hashtag_pool_size: hashtagPool.length,
